...
class Limelight(Subsystem):
	...
	def get_distance_to_target(self):
		# Convert to radians, since that's what math.tan expects
		ty = math.radians( self.nt_table.getNumber('ty', 0) )

		distance = (const.OUTER_PORT_TARGET_DISTANCE_FROM_GROUND - const.LIMELIGHT_MOUNT_HEIGHT) / math.tan(const.LIMELIGHT_MOUNT_ANGLE + ty) + const.LIMELIGHT_DISTANCE_FUDGE_INCHES
	...
	def get_main_target(self):
		x_ss = self.nt_table.getNumber( 'tx', None)
		y_ss = self.nt_table.getNumber( 'ty', None)
